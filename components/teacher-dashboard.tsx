...
import { Clock3, Code2, FileQuestion, Plus, Send, Sparkles, Trash2, UserPlus, WandSparkles } from 'lucide-react'
...
export function TeacherDashboard() {
  ...
  return <main dir="rtl" className="min-h-screen bg-background text-foreground">
    <header className="border-b border-border bg-card"><div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4"><div><p className="font-mono text-xs text-primary">EDU / AI WORKSPACE</p><h1 className="mt-1 text-xl font-bold tracking-tight sm:text-2xl">منصة المعلّم الذكية</h1></div><button onClick={() => setShowRegistration(true)} className="flex shrink-0 items-center gap-2 rounded-lg bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground"><UserPlus size={16}/> إنشاء حساب</button></div></header>
