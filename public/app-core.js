...
// الوضع العاجي الفاتح هو الافتراضي، مع احترام اختيار المستخدم للوضع الداكن.
...
function saveExamDraft(){
  ...
  showExamAlert('✅ تم إرسال الاختبا�� للطالب ��ع أزمنة منفصلة لكل سؤال.','success');showToast('🧪 تم إرسال الاختبار للطالب','success');
}
...
async function deleteGithubFile(){
  ...
  recordDevAudit({ status:'applied', request:'حذف ملف: '+path, summary:'حذف مف من المستودع بتأكيد صريح.', files:[{ path, commitUrl:r.commitUrl||null }], flagged:['��ذف ملف'] });
  refreshGithubSync();
  } else {
...
function clearDevAudit(){
  ...
  setData('devAuditLog', []);
  renderDevAudit();
  showToast('تم مسح س��ل الءءدقيق', 'info');
}
