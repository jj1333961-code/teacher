...
let proctorGlobalConfig=null;
function proctorLoadFaceMesh(){if(window.FaceMesh)return Promise.resolve();if(window.__faceMeshPromise)return window.__faceMeshPromise;window.__faceMeshPromise=new Promise((resolve,reject)=>{const sc=document.createElement('script');sc.src='https://cdn.jsdelivr.net/npm/@mediapipe/face_mesh@0.4/face_mesh.js';sc.crossOrigin='anonymous';sc.onload=()=>{window.FaceMesh?resolve():reject(new Error('facemesh-missing'))};sc.onerror=()=>reject(new Error('facemesh-load-failed'));document.head.appendChild(sc);setTimeout(()=>{window.FaceMesh?resolve():reject(new Error('facemesh-timeout'))},9000)});return window.__faceMeshPromise}
function getProctorSettings(){const ctx=proctor.context||{};const g=proctorGlobalConfig||{};const flag=(cv,gv,def)=>cv!==undefined?cv!==false:(gv!==undefined?gv!==false:def);const gazeSec=parseInt(ctx.proctorGazeGrace||ctx.proctorFaceGrace)||(g.gazeGraceMs?Math.round(g.gazeGraceMs/1000):12);const leaveSec=parseInt(ctx.proctorLeaveGrace)||(g.leaveGraceMs?Math.round(g.leaveGraceMs/1000):5);const touchSec=parseInt(ctx.proctorTouchGrace)||12;const maxV=parseInt(ctx.proctorMaxViolations)||g.maxViolations||2;return{touchGraceMs:Math.max(1,touchSec)*1000,gazeGraceMs:Math.max(3,gazeSec)*1000,leaveGraceMs:Math.max(1,leaveSec)*1000,maxViolations:Math.max(1,maxV),fullscreen:ctx.proctorFullscreen!==undefined?ctx.proctorFullscreen===true:g.fullscreen===true,focus:flag(ctx.proctorFocus,g.focus,true),touch:flag(ctx.proctorTouch,g.touch,true),autoRestore:flag(ctx.proctorAutoRestore,g.autoRestore,true),camera:flag(ctx.proctorEnabled,g.enabled,true),light:flag(ctx.proctorLight,g.light,true),face:flag(ctx.proctorFace,g.face,true),gaze:flag(ctx.proctorGaze,g.gaze,true),multiFace:flag(ctx.proctorMultiFace,g.multiFace,true)}}
...
function openProctorGate(context,onReady){proctorStopCamera();proctor.active=false;proctor.context=context;proctor.onReady=onReady;proctor.cancelled=false;proctor.stableSince=0;proctor.baseline=null;proctor.gazeSamples=[];proctor.eyeSamples=[];proctor.touches.clear();document.getElementById('proctorGate')?.classList.remove('hidden');setProctorCheck('proctorTouchCheck',isTouchDevice(),isTouchDevice()?'شاشة لمس جاهزة — يلزم إصبع واحد':'هذه المهمة تعمل على هاتف بشاشة لمس فقط');setProctorCheck('proctorLightCheck',false,'الإضاءة غير مفحوصة');setProctorCheck('proctorFaceCheck',false,'الوجه غير مفحوص');setProctorCheck('proctorGazeCheck',false,'العينان غير مفحوصتين');const hold=document.getElementById('proctorGateHold');if(hold){hold.setAttribute('aria-disabled','true');hold.classList.remove('holding');hold.textContent='بعد نجاح الفحص: ضع إصبع واحد هنا للبدء'}const st=getProctorSettings();const scanBtn=document.getElementById('proctorScanBtn');if(!st.camera){setProctorCheck('proctorLightCheck',true,'فحص الكاميرا معطّل');setProctorCheck('proctorFaceCheck',true,'فحص الوجه معطّل');setProctorCheck('proctorGazeCheck',true,'فحص النظر معطّل');if(scanBtn)scanBtn.style.display='none';if(hold){hold.setAttribute('aria-disabled','false');hold.textContent='الكاميرا معطّلة — ضع إصبعًا واحدًا هنا للبدء'}document.getElementById('proctorCameraStatus').textContent='فحص الكاميرا معطّل من الإعدادات — ضع إصبعًا واحدًا للبدء'}else{if(scanBtn)scanBtn.style.display='';document.getElementById('proctorCameraStatus').textContent='اضغط تشغيل الفحص للسماح بالكاميرا'}}
async function startProctorScan(){if(!navigator.mediaDevices?.getUserMedia){document.getElementById('proctorHelp').textContent='الكاميرا تحتاج متصفحاً حديثاً واتصال HTTPS.';return}try{proctor.stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:'user'},width:{ideal:640},height:{ideal:480}},audio:false});const video=document.getElementById('proctorVideo');video.srcObject=proctor.stream;await video.play();proctor.detector=null;proctor.detectorType='';if('FaceDetector' in window){try{proctor.detector=new FaceDetector({fastMode:true,maxDetectedFaces:2});proctor.detectorType='native'}catch(e){proctor.detector=null;proctor.detectorType=''}}if(proctor.detectorType!=='native'){try{if(!window.FaceMesh)await proctorLoadFaceMesh();if(window.FaceMesh){const mesh=new FaceMesh({locateFile:file=>'https://cdn.jsdelivr.net/npm/@mediapipe/face_mesh@0.4/'+file});mesh.setOptions({maxNumFaces:2,refineLandmarks:true,minDetectionConfidence:.55,minTrackingConfidence:.55});mesh.onResults(results=>{proctor.faceMeshResults=results.multiFaceLandmarks||[]});proctor.detector=mesh;proctor.detectorType='mediapipe'}}catch(e){console.warn('[v0] تعذر تحميل نموذج الوجه، سيستمر الفحص بالإضاءة واللمس فقط',e)}}if(proctor.detectorType!=='native'&&proctor.detectorType!=='mediapipe'){proctor.detectorType='none';document.getElementById('proctorHelp').textContent='تعذر تحميل نموذج فحص الوجه — يستمر الفحص بالإضاءة واللمس. تحقق من الإنترنت لتفعيل فحص الوجه.'}else{document.getElementById('proctorHelp').textContent='يعمل الفحص على Chrome وSafari وFirefox وEdge الحديثة.'}document.getElementById('proctorScanBtn').disabled=true;proctor.scanTimer=setInterval(proctorAnalyzeFrame,350);proctorAnalyzeFrame()}catch(e){proctorStopCamera();const name=e&&e.name||'';document.getElementById('proctorHelp').textContent=name==='NotAllowedError'?'تم رفض إذن الكاميرا. اسمح به من إعدادات الموقع ثم أعد المحاولة.':name==='NotFoundError'?'لم يتم العثور على كاميرا متاحة.':name==='NotReadableError'?'الكاميرا مستخدمة في تطبيق آخر. أغلقه ثم أعد المحاولة.':name==='SecurityError'?'افتح الصفحة عبر HTTPS للسماح بالكاميرا.':'تعذر فتح الكاميرا. تحقق من إذن المتصفح ثم أعد المحاولة.'}}
...
async function loadGlobalProctorSettings(){try{const res=await fetch('/api/anti-cheat',{cache:'no-store'});const data=await res.json();const global=data.global||{};const config=global.config||{};proctorGlobalConfig={...config,enabled:global.enabled!==false};const set=(id,value)=>{const el=document.getElementById(id);if(el&&value!==undefined)el.type==='checkbox'?el.checked=Boolean(value):el.value=value};set('globalProctorEnabled',global.enabled!==false);set('globalLightEnabled',config.light!==false);set('globalFaceEnabled',config.face!==false);set('globalGazeEnabled',config.gaze!==false);set('globalMultiFaceEnabled',config.multiFace!==false);set('globalFocusEnabled',config.focus!==false);set('globalTouchEnabled',config.touch!==false);set('globalAutoRestore',config.autoRestore!==false);set('globalFullscreen',config.fullscreen===true);set('globalGazeGrace',Math.round((Number(config.gazeGraceMs)||12000)/1000));set('globalLeaveGrace',Math.round((Number(config.leaveGraceMs)||5000)/1000));set('globalMaxViolations',config.maxViolations||2)}catch(e){console.warn('[v0] global proctor settings unavailable',e)}}
function applyGlobalProctorSettings(){const enabled=document.getElementById('globalProctorEnabled')?.checked!==false;const light=document.getElementById('globalLightEnabled')?.checked!==false;const face=document.getElementById('globalFaceEnabled')?.checked!==false;const gazeChk=document.getElementById('globalGazeEnabled')?.checked!==false;const multiFace=document.getElementById('globalMultiFaceEnabled')?.checked!==false;const focus=document.getElementById('globalFocusEnabled')?.checked!==false;const touch=document.getElementById('globalTouchEnabled')?.checked!==false;const autoRestore=document.getElementById('globalAutoRestore')?.checked!==false;const gaze=Math.max(3,Math.min(120,parseInt(document.getElementById('globalGazeGrace')?.value)||12));const leave=Math.max(1,Math.min(60,parseInt(document.getElementById('globalLeaveGrace')?.value)||5));const max=Math.max(1,Math.min(10,parseInt(document.getElementById('globalMaxViolations')?.value)||2));const fullscreen=document.getElementById('globalFullscreen')?.checked===true;const config={enabled,light,face,gaze:gazeChk,multiFace,focus,touch,gazeGraceMs:gaze*1000,leaveGraceMs:leave*1000,maxViolations:max,fullscreen,autoRestore};proctorGlobalConfig={...config};const allItems=[].concat(recordElements||[],homeworkItems||[],readingItems||[]);allItems.forEach(function(el){el.proctorEnabled=enabled;el.proctorLight=light;el.proctorFace=face;el.proctorGaze=gazeChk;el.proctorMultiFace=multiFace;el.proctorFocus=focus;el.proctorTouch=touch;el.proctorGazeGrace=gaze;el.proctorFaceGrace=gaze;el.proctorLeaveGrace=leave;el.proctorMaxViolations=max;el.proctorFullscreen=fullscreen;el.proctorAutoRestore=autoRestore});fetch('/api/anti-cheat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'save-global',enabled,config})}).catch(function(){/* local item settings remain usable when database is unavailable */});renderRecordElements();renderExtraElements();renderHomeworkItems();renderReadingItems();showToast('تم تطبيق إعدادات الفحص على جميع عناصر التسميع والاختبار والواجب','success')}
