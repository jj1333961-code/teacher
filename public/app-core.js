...
function saveSession(isFinal) {
  ...
  if(activeElements.length === 0) { showToast('⚠️ لم يتم الحفظ — لا يوجد عنصر تسميع', 'error'); return; }
...
function renderAdminExamHistory(s) {
  ...
  exams.forEach(function(ex,examIndex){
    ...
    questions.forEach(function(q,i){
      ...
      if(image)h+='<div style="margin-top:10px"><strong>سورة/مرجع السؤال:</strong><br><img src="'+escapeHtml(image)+'" alt="مرجع السؤال '+(i+1)+'" loading="lazy" style="max-width:100%;border-radius:10px;border:1px solid var(--border)"></div>';
...
async function refreshGithubSync(){
  ...
  if(box) box.innerHTML = '<p style="color:var(--text-light)">جارٍ التحقق من الحال بمستودعك...</p>';
  try {
...
function toggleShareWithParent(idx) {
  ...
  setData('messages', msgs);
  renderMessages();
  alert(msgs[idx].shareWithParent ? '✅ أصبح بإمكان ولي الأمر الاطلاع على هذا الملف (بدون تعديل).' : '🚫 تم منع ولي الأمر من الاطلاع على هذا الملف.');
}
...
async function verifyAndSubmitRecitation(taskIdx, blob, dataUrl, transcript, aiBoxId, statusEl, fileName) {
  ...
  if(matchPct === null) {
    showToast('❌ لا يوجد بصمة صوتية مرجعية صالحة لهذا الطالب — لم يتم حفظ أو إرسال التسجيل', 'error');
  ...
  if(aiBox) aiBox.innerHTML = '<div class="alert alert-success"><strong>✅ تقرير الذكاء الاصطناعي:</strong><br>' +
...
function generateAIResponse(text, student) {
  ...
  if(has('اية','آية','ايات','آيات','صورة','اقرأ','مصحف')) {
    return '📖 لعرض الآيات المطلوبة منك:<br>1. افتح <strong>المهام المطلوبة</strong> في صفحتك.<br>2. اضغط <strong>📖 عرض الآيات بحجم كبير</strong> في المهمة.<br>3. استخدم زرار ➕ / ➖ للتكبير والتصغير حتى تصل لأوضح حجم لعينيك.<br><br>الآيات تُعرض برسم العثماني المشكَّل كصورة مطابقة تماماً لمصحف.';
  }
  // البصمة الصوتية
  ...
  // التواصل مع المسؤو
  if(has('مسؤول','معلم','شيخ','ابلاغ','إبلاغ','رسائل','رسالة','تواصل','شكوى')) {
  ...
  // افتراضي ذكي
  let r = '🤖 أهلاً '+name+'، لم أفهم سؤالك تماماً، لكني أستطيع مساعدتك فوراً في:<br>';
