...
async function saveAllDataToNeon(){
  ...
  try{ const res=await fetch('/api/data',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({data:collectCloudData()})}); if(!res.ok)throw new Error(); if(status)status.textContent='محفوظ في Neon'; }
...
function openProctorGate(context,onReady){proctorStopCamera();proctor.active=false;proctor.context=context;proctor.onReady=onReady;proctor.cancelled=false;proctor.stableSince=0;proctor.riskScore=0;proctor.riskUpdatedAt=Date.now();proctor.baseline=null;proctor.gazeSamples=[];proctor.eyeSamples=[];proctor.touches.clear();document.getElementById('proctorGate')?.classList.remove('hidden');setProctorCheck('proctorTouchCheck',true,isTouchDevice()?'الجهاز اللمسي جاهز — استخدم إصبعًا واحدًا':'الجهاز غير لمسي — يمكنك استخدام الماوس أو لوحة اللمس');setProctorCheck('proctorLightCheck',false,'الإضاءة غير مفحوصة');setProctorCheck('proctorFaceCheck',false,'الوجه غير مفحوص');setProctorCheck('proctorGazeCheck',false,'العينان غير مفحوصتين');const hold=document.getElementById('proctorGateHold');if(hold){hold.setAttribute('aria-disabled','true');hold.classList.remove('holding');hold.textContent='بعد نجاح الفحص: ضع إصبع واحد هنا للبدء'}document.getElementById('proctorCameraStatus').textContent='اضغط تشغيل الفحص للسماح بالكاميرا'}
async function startProctorScan(){if(!navigator.mediaDevices?.getUserMedia){document.getElementById('proctorHelp').textContent='الكاميرا تحتاج متصفحاً حديثاً واتصال HTTPS.';return}try{proctor.stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:'user'},width:{ideal:640},height:{ideal:480}},audio:false});const video=document.getElementById('proctorVideo');video.srcObject=proctor.stream;await video.play();if('FaceDetector' in window){proctor.detector=new FaceDetector({fastMode:true,maxDetectedFaces:2});proctor.detectorType='native'}else if(window.FaceMesh){const mesh=new FaceMesh({locateFile:file=>'https://cdn.jsdelivr.net/npm/@mediapipe/face_mesh@0.4/'+file});mesh.setOptions({maxNumFaces:2,refineLandmarks:true,minDetectionConfidence:.55,minTrackingConfidence:.55});mesh.onResults(results=>{proctor.faceMeshResults=results.multiFaceLandmarks||[]});proctor.detector=mesh;proctor.detectorType='mediapipe'}else{document.getElementById('proctorHelp').textContent='فحص الوجه غير متاح في هذا المتصفح. يمكنك تحديث المتصفح أو استخدام جهاز يدعم الكاميرا، ولن يتم تجميد الصفحة.';proctorStopCamera();return}document.getElementById('proctorScanBtn').disabled=true;document.getElementById('proctorHelp').textContent='يعمل الفحص على Chrome وSafari وFirefox وEdge الحديثة.';proctor.scanTimer=setInterval(proctorAnalyzeFrame,350);proctorAnalyzeFrame()}catch(e){proctorStopCamera();const name=e&&e.name||'';document.getElementById('proctorHelp').textContent=e&&e.message==='face-model-unavailable'?'تعذر تحميل نموذج فحص الوجه. تحقق من اتصال الإنترنت ثم أعد المحاولة.':name==='NotAllowedError'?'تم رفض إذن الكاميرا. اسمح به من إعدادات الموقع ثم أعد المحاولة.':name==='NotFoundError'?'لم يتم العثور على كاميرا متاحة.':name==='NotReadableError'?'الكاميرا مستخدمة في تطبيق آخر. أغلقه ثم أعد المحاولة.':name==='SecurityError'?'افتح الصفحة عبر HTTPS للسماح بالكاميرا.':'تعذر فتح الكاميرا. تحقق من إذن المتصفح ثم أعد المحاو��ة.'}}
...
function showPage(id) {
  ...
  if(id === 'addStudent') { renderSubjectSelect(); initJuzSelect(); voiceBlob = null; voiceFingerprint = null; voiceDataUrl = null; voiceProfileGemini = null; document.getElementById('voicePreview').style.display='none'; document.getElementById('voiceRecordStatus').textContent='اضغط للتسجي (20 ثانية)'; }
...
function renderStudentSpeechPreview(parsed,filled){
  ...
  const labels={name:'اسم الطالب',username:'اسم المستخدم',national:'الرقم القومي',phone:'رقم الهاتف',birth:'تاريخ ال��يلاد',parent:'اسم ولي الأمر',notes:'الملاحظات'};
...
async function toggleStudentIntakeRecord(){
  ...
  if(typeof MediaRecorder==='undefined'){status.textContent='هذا المتصفح لا يدعم التسجيل الصوتي. ��ستخدم إصداراً حديثاً من Chrome أو Safari.';return}
  try{
...
async function toggleVoiceRecord() {
  ...
  const allowMic = confirm('🔴 يرجى السماح للموقع بالوصول إلى الميكروفون لتسجل البصمة الصوتية.\n\nاضغط "موافق" ثم اختر "السماح" في نافذة المتصفح.');
...
// ====== شاشة القفل الموحدة: توجه لقائي للصفحة المناسبة ======
...
function openEdit(id) {
  ...
  const evs=document.getElementById('editVoiceStatus'); if(evs)evs.textContent=(s.voiceProfile||s.voicePrintVec)?('البصمة الصوتية موجودئ'+(s.voiceProfile?' (جميناي)':'')+'. يمكنك استبدالها بتسيل جديد.'):'لا توجد بصمة صوتية محفوظة حالياً.';
...
function renderRecordElementHTML(el, i, num) {
  ...
  html += '<button class="btn btn-xs btn-info" onclick="duplicateRecordElement('+i+')" title="تضيف (إنشاء نسخة أخرى)">📑 تضعيف</button>';
...
async function loadGlobalProctorSettings(){try{const res=await fetch('/api/anti-cheat',{cache:'no-store'});const data=await res.json();const global=data.global||{};const config=global.config||{};const set=(id,value)=>{const el=document.getElementById(id);if(el&&value!==undefined)el.type==='checkbox'?el.checked=Boolean(value):el.value=value};set('globalProctorEnabled',global.enabled!==false);set('globalFocusEnabled',config.focus!==false);set('globalTouchEnabled',config.touch!==false);set('globalAutoRestore',config.autoRestore!==false);set('globalFullscreen',config.fullscreen===true);set('globalGazeGrace',Math.round((Number(config.gazeGraceMs)||12000)/1000));set('globalLeaveGrace',Math.round((Number(config.leaveGraceMs)||5000)/1000));set('globalMaxViolations',config.maxViolations||2);set('globalLightMin',config.lightMin||42);set('globalLightMax',config.lightMax||240);set('globalRequireFace',config.requireFace!==false);set('globalRequireEyes',config.requireEyes!==false)}catch(e){console.warn('[v0] global proctor settings unavailable',e)}}
function applyGlobalProctorSettings(){const enabled=document.getElementById('globalProctorEnabled')?.checked!==false;const focus=document.getElementById('globalFocusEnabled')?.checked!==false;const touch=document.getElementById('globalTouchEnabled')?.checked!==false;const autoRestore=document.getElementById('globalAutoRestore')?.checked!==false;const gaze=Math.max(3,Math.min(120,parseInt(document.getElementById('globalGazeGrace')?.value)||12));const leave=Math.max(1,Math.min(60,parseInt(document.getElementById('globalLeaveGrace')?.value)||5));const max=Math.max(1,Math.min(10,parseInt(document.getElementById('globalMaxViolations')?.value)||2));const fullscreen=document.getElementById('globalFullscreen')?.checked===true;const lockMode=document.getElementById('globalLockMode')?.value||'theme';const aiAnalysis=document.getElementById('globalAIAnalysis')?.checked!==false;const lightMin=Math.max(10,Math.min(240,parseInt(document.getElementById('globalLightMin')?.value)||42));const lightMax=Math.max(lightMin+1,Math.min(255,parseInt(document.getElementById('globalLightMax')?.value)||240));const requireFace=document.getElementById('globalRequireFace')?.checked!==false;const requireEyes=document.getElementById('globalRequireEyes')?.checked!==false;const touchGrace=Math.max(1,Math.min(60,parseInt(document.getElementById('globalTouchGrace')?.value)||12));const config={enabled,focus,touch,touchGrace,gazeGrace:gaze,leaveGrace:leave,gazeGraceMs:gaze*1000,leaveGraceMs:leave*1000,maxViolations:max,fullscreen,autoRestore,lockMode,aiAnalysis,lightMin,lightMax,requireFace,requireEyes};try{localStorage.setItem('thimar_global_proctor',JSON.stringify(config))}catch(e){}const allItems=[].concat(recordElements||[],homeworkItems||[],readingItems||[]);allItems.forEach(function(el){el.proctorEnabled=enabled;el.proctorFocus=focus;el.proctorTouch=touch;el.proctorGazeGrace=gaze;el.proctorFaceGrace=gaze;el.proctorLeaveGrace=leave;el.proctorMaxViolations=max;el.proctorFullscreen=fullscreen;el.proctorAutoRestore=autoRestore});fetch('/api/anti-cheat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'save-global',enabled,config})}).catch(function(){/* local item settings remain usable when database is unavailable */});renderRecordElements();renderExtraElements();renderHomeworkItems();renderReadingItems();showToast('تم تطبيق إعدادات الفحص على جميع عناصر التسميع والاختبار والواجب','success')}
...
function renderReadingItems() {
  ...
  readingItems.forEach((item, i) => {
    ...
    html += '<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;"><strong style="color:var(--primary)">📖 قراءة ضافية ' + (i+1) + '</strong><button class="btn btn-xs btn-danger" onclick="removeReadingItem('+i+')">🗑️ حذف</button></div>';
    ...
    html += '<div class="record-options" style="margin-top:8px;"><div class="record-option"><input type="checkbox" id="rdShow_'+i+'" '+(item.showAyat?'checked':'')+' onchange="updateReadingItem('+i+', \'showAyat\', this.checked)"><label for="rdShow_'+i+'">👁️ إظهار صورة الآيات للطالب</label></div><small style="color:var(--text-light)">فحص الغش لهذه القراءة يُدار من الإعدادات الموحدة أعلى صفحة التسميع.</small></div>';
...
function renderExamPlanRows(){
  ...
  examPlanRows.forEach((r,i)=>{
    ...
    h+='<div class="exam-plan-row" style="border:1px solid var(--border);padding:12px;border-radius:10px;margin-bottom:10px;'+(weak?'opacity:.65;background:rgba(245,158,11,.08);':'')+'">'+
      ...
      (r.type==='mcq'?'<div class="form-group"><label>عدد الاخت��ارا��</label><input type="number" min="2" max="6" value="'+(r.optionsCount||4)+'" onchange="examPlanRows['+i+'].optionsCount=Math.max(2,Math.min(6,parseInt(this.value)||4))"></div>':'')+
      ...
      '<div class="form-group"><small style="color:var(--text-light)">حماية الاختبار تُدار من إعدادات فحص الغش الموحدة في أعلى صفحة التسميع.</small></div>'+
...
function renderStudentExam(){
  ...
  if(!ex||ex.status!=='pending'){const latest=(s&&s.examResults||[]).slice(-1)[0];c.innerHTML=latest?renderStudentResultHtml(latest):'<div class="alert alert-info">لا يوجد اختبار نشط حاياً.</div>';return}
  ...
  const requiredQuestion=ex.questions[studentExamCurrentIndex],questionAuthId=ex.id;
  if(getProctorSettings().enabled!==false&&(proctorExamAuthorizedId!==questionAuthId||!proctor.active)){c.innerHTML='<div class="alert alert-info"><h3>التحقق الأمني قبل بدء الاختبار</h3><p>يُجرى الفحص مرة واحدة عند بداية الاختبار فقط، ثم يمكنك الانتقال بين الأسئلة دون إعادة فتح الكاميرا.</p><button class="btn btn-primary" onclick="openProctorGate({type:\'exam\',id:\''+escapeHtml(ex.id)+'\'},function(){proctorExamAuthorizedId=\''+escapeHtml(questionAuthId)+'\';renderStudentExam()})">بدء فحص الاختبار</button></div>';return}
  ...
  let h='<div class="exam-sticky"><strong>اختبار '+escapeHtml(ex.baseSurah?'بعد سورة '+ex.baseSurah:'')+'</strong><div style="margin-top:6px">السؤال الحالي '+(studentExamCurrentIndex+1)+' من '+ex.questions.length+' — <span id="studentExamTimer" class="exam-timer">الوقت: '+formatExamTime(remaining)+'</span></div><div>لكل سؤال وقت مستقل. الشائح السابقة للعرض فقط، والقادمة تُفتح بعد تأكيد الإجابة.</div></div>';
...
async function recordStudentExamAudio(i){
  ...
  try{
    ...
    recorder.onstop=async()=>{
      ...
      aiBox.innerHTML='<div class="alert alert-info">🔒 تم حفظ إجاة التسجيل ونتيجة التحليل داخلياً. لن تظهر نتيجة الذكاء الاصطناعي للطالب حتى يقرر المسؤول نشرها.</div>';
...
function renderStudentResultHtml(ex){if(ex.status==='pending_audio_review')return '<div class="alert alert-warning"><h3>تم استلام الاختئار</h3><p>النتيجة معلّقة لحين مراجعة المئؤول للتسجيل الصوتي.</p></div>';let h='<div class="alert alert-success"><h3>النتيج النهائية: '+ex.score+' / '+ex.maxScore+'</h3><p>تم حفظ جميع إجاباتك وإرسالها للمسؤول.</p></div>';if(ex.questions&&ex.answers){h+='<details open><summary>مراجعة إجاباتك</summary>';ex.questions.forEach((q,i)=>{const a=ex.answers[i]||{};h+='<div class="task-card"><strong>س'+(i+1)+':</strong> '+escapeHtml(q.prompt||'')+'<br>إجابتك: '+escapeHtml(a.answer||'—')+'<br>القييم: '+(a.score>=1?'صحيحة':'غير صحيحة')+'</div>'});h+='</details>'}return h}
...
function saveSession(isFinal) {
  ...
  if(activeElements.length === 0) { showToast(' لم يتم الحفظ — لا يوجد نصر تسمع', 'error'); return; }
  ...
  // الحف�� النهائي: نقل مهام اليوم إلى الأرشيف ثم تفريغ المهام الالية والانتقال لليوم التالي.
...
// إعاة الصفحة لحالتها الأصلية بعد الحفظ النهائي
...
function renderAdminExamHistory(s) {
  ...
  exams.forEach(function(ex,examIndex){
    ...
    h+='<div class="history-day-header" style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap"><span>اتبار '+(exams.length-examIndex)+' — '+formatExamDate(submitted)+'</span><span class="score-badge">'+score+' / '+maxScore+' ('+percent+'٪)</span></div>';
    ...
    questions.forEach(function(q,i){
      ...
      const answerScore=Number(a.score)||0,answerLabel=answerScore>=1?'ص��يحة':answerScore>0?'جزئية':'غير صحيحة';
  ...
  if(finalizedSessions.length === 0) {
    ...
    Object.keys(grouped).sort().reverse().forEach(date => {
      ...
      grouped[date].forEach(sess => {
        ...
        if(sess.finalizedAt) html += '<p style="margin-top:8px; color:var(--success); font-size:0.9rem;">✅ تم الإغلاق النهائي تريخ: '+sess.finalizedAt+'</p>';
...
function openFullChart() {
  ...
  if(finalizedSessions.length === 0) {
    alert('لا توجد تسميعات نهائية للعرض بد'); return;
  }
...
function renderVoiceBox(key) {
  ...
  if(v) {
    ...
    h += '<button class="btn btn-xs btn-danger" onclick="clearVoiceMsg(\''+key+'\')">ء️ حذف</button>';
  }
  h += '</div>';
  box.innerHTML = h;
}
...
function sendUserMessageToAdmin(role) {
  ...
  if(!text && !voice) { showToast('❌ اكتب رسالة أو سجّل رسالة صوية أولاً', 'error'); return; }
...
async function refreshGithubSync(){
  ...
  if(btn){ btn.disabled = true; btn.textContent = '⏳ جرٍ التحقق...'; }
  if(box) box.innerHTML = '<p style="color:var(--text-light)">جارٍ التحق من التال بمستودعك...</p>';
  try {
...
// ===== مساعد تطوير الموقع بالذكاء الاصطاعي (للمسؤول فقط) =====
...
// عبرات تدل على عمليات خطيرة تتطلب تأكيداً صريحاً قبل التنفيذ التلقائي.
const DEV_DANGER_PATTERNS = [
  ...
  {re:/حذف\s*(الطلب|تسجيلات\s*الطلاب|بيانات\s*الطلب)/i, label:'حذف الطلاب أو تسجيلاتهم'},
  {re:/(المصادق��|تسجيل\s*ادخول|نظام\s*الدخول|كلمة\s*المرور\s*الرئيسية|authentication|login\s*system)/i, label:'تغيير نظام المصادقة'},
...
function clearDevAudit(){
  ...
  setData('devAuditLog', []);
  renderDevAudit();
  showToast('تم مسح سجل الدقيق', 'info');
}
...
function renderDevPlan(plan){
  ...
  if(plan.applied){
    ...
    // 2) ملخص التغييرات
    h += '<div style="margin-bottom:6px;"><strong>🧾 ملخص التغييرات:</strong> '+esc(plan.summary || 'تم تطيق التعديلات البرمجية المطلوبة بنجاح.')+'</div>';
...
function editSubject(id) {
  ...
  if(newPhone && newPhone.length !== 11) return alert('رقم الهاتف يجب أن يون 11 رقم');
...
function renderStudentDashboard() {
  ...
  html += '<p><strong>المدرس:</strong> '+(s.subjects ? s.subjects.map(sub => sub.teacher+(sub.phone ? ' (📞 '+sub.phone+')' : '')).join('<br>') : '-')+'</p>';
  ...
  if(s.activeExam && s.activeExam.status==='pending'){
    document.getElementById('studentInfo').innerHTML += '<div class="alert alert-warning" style="margin-top:15px;border:2px solid var(--warning);font-size:1.05rem"><strong>🔔 لديك اختبار جيد!</strong><br>أرسه المسؤئل ويجب حله. <button class="btn btn-warning" style="margin-top:8px" onclick="showPage(\'studentExamPage\')">🧪 فتح الاختبار الآن</button></div>';
  ...
  if(draft && isQuran) {
    ...
    draft.elements.forEach((el, ei) => {
      ...
      draftHtml += 'التقيم: <span class="badge '+getRatingClass(el.rating)+'">'+getRatingLabel(el.rating)+'</span>';
      draftHtml += '</div>';
    });
    const timeLeft = Math.max(0, 24 - ((Date.now() - draft.draftCreatedAt) / (60 * 60 * 1000)));
...
function renderStudentCompletedTasks() {
  ...
  completed.slice().reverse().forEach(task => {
    ...
    html += '<p style="color:var(--text-light); font-size:0.9rem;">🕐 تمت الموفقة: '+task.approvedAt+'</p>';
    html += '</div>';
  });
  html += '</div>';
  html += renderTaskArchiveHtml(s);
...
function studentAyatBlock(task, key) {
  ...
  h += '<button class="btn btn-xs btn-secondary" onclick="toggleInlineAyat(\'stAyat_'+key+'\', \''+task.surah+'\', \''+task.from+'\', \''+(task.to||task.from)+'\')">👁️ إظ��ار/إخفاء الآيات</button>';
...
function renderStudentTasks() {
  ...
  allTasks.forEach((task, originalIdx) => {
    ...
    if(task.type === 'homework') {
      ...
      if(isRejected) {
        ...
        html += '<div style="margin-top:10px;"><div class="file-upload" onclick="document.getElementById(&quot;hwFile_'+originalIdx+'&quot;).click()"><div>📷 اضغط لرفع صرة الواجب الجديدة</div></div>';
        ...
        html += '<div style="margin-top:10px;"><div class="file-upload" onclick="document.getElementById(&quot;hwFile_'+originalIdx+'&quot;).click()"><div>📷 اضغط لرع صورة الاجب</div></div>';
        html += '<input type="file" id="hwFile_'+originalIdx+'" accept="image/*" style="display:none" onchange="uploadTaskFile('+originalIdx+', this, &quot;homework&quot;)"></div>';
      } else {
      ...
      if(isProctorCancelled) {
        html += '<div class="task-rejected-alert">لا يمكن إعادة هذه الهمة بعد إلغائها. تواصل مع المسؤول لإسناد مهمة جديدة.</div>';
      } else if(isRejected) {
      ...
      if(isProctorCancelled) {
        html += '<div class="task-rejected-alert">ل يمكن إعادة هذه لمهمة بعد إلغئها. تواصل مع المؤول لإسنا مهمة جديدة.</div>';
      } else if(isRejected) {
  ...
  document.getElementById('studentTasksSection').innerHTML='<div class="student-work-grid" style="margin-top:20px"><section class="page student-work-card"><h4 style="color:var(--primary)">الاختبارات <span class="badge badge-primary">'+(exam?1:0)+'</span></h4>'+examHtml+'</section><section class="page student-work-card"><h4 style="color:var(--success)">التسميع <span class="badge badge-success">'+(recitationTasks?temp.querySelectorAll('[data-task-category="recitation"]').length:0)+'</span></h4>'+(recitationTasks||'<p style="color:var(--text-light)">لا توجد مهمات تسميع حالية.</p>')+'</section><section class="page student-work-card"><h4 style="color:var(--warning)">المهمات اإضافية <span class="badge badge-warning">'+(extraTasks?temp.querySelectorAll('[data-task-category="extra"]').length:0)+'</span></h4>'+(extraTasks||'<p style="color:var(--text-light)">لا توجد مهمت ضافية حالية.</p>')+'</section></div>';
...
function studentVoiceRecorderHTML(idx) {
  return '<div style="margin-top:10px; display:flex; gap:10px; align-items:center; flex-wrap:wrap;">' +
    ...
    '<span id="stVoiceStatus_'+idx+'">اضغط لبدء التسجيل</span>' +
...
// إرسال الصوت إلى Gemini على الخادم للتفريغ وتحيح التلاوة.
...
// التحق الكامل (بصمة صوتية + محتوى ائتلاوة) ثم الإرسال أو الرفض
async function verifyAndSubmitRecitation(taskIdx, blob, dataUrl, transcript, aiBoxId, statusEl, fileName) {
  ...
  if(matchPct !== null && matchPct < VOICE_MATCH_THRESHOLD) {
    showToast('❌ فشل التحقق من البصمة الصوتية (' + matchPct + '%) — لم يتم حفظ أو إرسال التجيل', 'error');
  ...
  if(aiBox) aiBox.innerHTML = '<div class="alert alert-success"><strong>✅ تقرير الذكاء ااصطنعي:</strong><br>' +
...
// رفع ملف تسجيل صوتي جاهز من جهاز الالب (يمر بنفس التحقق)
...
// ====== جمي��اي هو المسؤوئ الوحيد عن البصمة الصوتية والتحقق من هوية المتحدث ======
...
async function verifyVoiceIdentity(blob,student){
  ...
  if(!profile)throw new Error('لا توجد بصمة Gemini مرجعية محفوة لهذا الطال');
  const audio=await voiceAudioPayload(blob);
...
function renderStudentChart() {
  ...
  const finalizedSessions = sessions.filter(sess => !sess.isDraft);
  if(finalizedSessions.length === 0) {
    document.getElementById('studentChartSection').innerHTML = '<div class="page" style="margin-top:20px;"><h4 style="color:var(--primary); margin-bottom:15px;">📊 مخطط الت��ييم</h4><div class="alert alert-info">لا توجد تقييم��ت نهائية مسجلة بعد. سيتم ظهور المخطط بعد إغلاق أول تسميع.</div></div>'; return;
  }
...
function uploadFile(input) {
  ...
  reader.onload = function(e) {
    ...
    const fileData = {
      ...
      uploadedBy: currentUser ? (currentUser.mobile || currentUser.name || 'مسؤول') : 'مؤول'
    };
    files.push(fileData);
...
function generateParentWelcome(student) {
  ...
  if(last) {
    ...
    if(last.totalScore >= 14) base.body += 'أداء متاز! ابنك يُظهر تفوقاً واضحاً. استمر في تشجيعه. 🌟';
...
function generateAIResponse(text, student) {
  ...
  if(has('السلا����','مرحبا','مرحباً','هلا','اهلا','أهلا','صباح','مساء')) {
    return 'وعليكم السلام ورحمة الله وبركاته '+name+'! ء<br><br>أنا <strong>مساعدك الذكي</strong> في رحلتك مع القرآن، متاح ئك 24 ساعة.<br>جرّب أن تكتب:<br>• <em>مستواي</em> — لعرض آخر تقييم وتحليله<br>• <em>مها</em> — لعرض الواجبءت والتسجيلات المطلوبة<br>• <em>الآيات</em> — لمعرفة كيف ترى آيات تسيعك كصورة<br>• <em>خطة</em> — لخطة حفظ يومية م��صصة ك<br>• <em>تحفيز</em> — لجرعة همة 💪';
  }
  // شكر
  if(has('شكرا','شكراً','جزاك','بارك الله','تمام','ok')) {
    return 'وإياك '+name+' 🌸 دائماً في خدمتك. استمر، فكل حرف تحظه لك به حسنة والحسنة بعشر أمثالها.';
  }
  // المستوى / التقييم
  if(has('مستوى','مستواي','تقييم','درجات','درجة','نتيجة','نتيجتي')) {
    if(!last) return 'مرحباً '+name+'! 🌟<br><br>لم يُسجَّل أي تسميع نهائي بعد. ابدأ اليوم، وسيظهر تقييك هنا فور اعتماد من المسؤول.';
  ...
  if(has('واجب','مهام','مهمة','مطلوب','تسجيل صوتي','راءة')) {
    if(pending.length === 0) return '🎉 لا توجد مهام معلّقة الآن يا '+name+'. استل الوقت في مراجعة ما حفظته سابقاً.';
  ...
  if(has('اية','آية','ايات','آيات','صورة','اقرأ','مصحف')) {
    return '📖 لعرض الآيات المطلوبة منك:<br>1. افتح <strong>المهام المطلوبة</strong> في صفحتك.<br>2. اضغط <strong>📖 عرض الآيات بحجم كبير</strong> في المهمة.<br>3. استخدم زرار ➕ / ➖ للتكبير والتصغير حتى تصل لأوح حجم لعينيك.<br><br>لآيات تُعرض بارم العثمان المشكَّل كصورة مطابقة تماماً لمصحف.';
  }
  // البصمة الصوتية
  if(has('بصمة','صوتي','صت','ميكروفون','تحق')) {
    return '🎙️ <strong>البصمة الصوتية:</strong> عند تسجيلك أول مرة حُفظت بصمة صوتك في النظام. عند إرسالك أي تسجيل، يحلله الذكاء الاصطناعي ويطابقه ببصمتك تلقائياً، ولا يُقبل التسجيل إلا إذا كان صوك أنت.<br><br>لأفضل نتيجة: سجّل في مكان هادئ، وقرّب الميكروفون، وتحدث بصوت طبيعي واضح.';
  }
  // خطة الحفظ
  ...
  if(has('تحفيز','همة','ملل','تعبان','زهقن','احبت','أحبطت')) {
    const quotes = [
      '🌟 «وَلَقَدْ يَسَّرْنَا الْقُْآنَ لِلذِّكْرِ فَهَلْ مِن مُّدَّكِرٍ» — الحفظ أيسر مما تظن، ابدأ فط.',
  ...
  if(has('مسؤول','معلم','شيخ','ابلاغ','إبلاغ','رسال','رسالة','تواصل','شكوى')) {
    return ' يمكنك مراسلة المسؤول مباشرة من <strong>صندوق الرسائ</strong> في صفحتك، وسيصلك الرد هناك مع إشعار. إن كان الأم�� عاجلاً اذكر كلمة "عال" في بداية رسالتك.';
  }
  // ديني عام
  ...
  r += '• <strong>مستواي</strong> — تحليل آخر تقييم<br>• <strong>مهامي</strong> — الواجبات والتسجيلات<br>• <strong>تقدمي</strong> — إحصائيات وتطورك<br>• <strong>خطة</strong> — جدول حفظ يومي<br>• <strong>الآيات</strong> — كيف تعرض آيات التسيع كصورة<br>• <strong>تحفيز</strong> — كلمة تشدّ همتك';
