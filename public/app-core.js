  ...
  if (params.get('google') === 'success') {
    fetch('/api/auth/google/session', { cache: 'no-store', credentials: 'same-origin' })
      ...
      .catch(function() {
        ...
        if (box) box.innerHTML = '<div class="alert alert-danger">تعذر استكمال تسجيل الدخول عبر Google.</div>';
      });
    return;
  }
  if (restoreSession()) {
