...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (mode === "generate_exam") {
      ...
      if (safeQuestions.length < requestedCount) return json({ error: `لم يجتز فحص الجودة سوى ${safeQuestions.length} من ${requestedCount} أسئلة. ستتم إعادة التوليد بمصدر احتياطي أو جرّب توسيع النطاق.`, diagnostics, retryable: true }, 502)
