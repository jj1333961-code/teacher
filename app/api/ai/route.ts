...
- لا تستخدم مطلقاً سورة يقل عدد آياتها عن 20، حتى إن وردت في النطاق المطلوب.
- جميع الأسئلة شديدة الصعوبة ومحترفة، وتعتمد على المتشابهات اللفظية الحقيقية: فروق الواو والفاء، الزيادة والنقص، اختلاف الضمائر والمفرد والجمع، اختلاف بدايات الآيات وخواتيمها، والتمييز بين موضعين متقاربين. يجب أن يبقى لكل سؤال جواب واحد قطعي.
- إذا كان type=complete فاختر حد بداية وحد نهاية حقيقيين لصيغة «اكتب من قوله تعالى … إلى قوله تعالى …»، واجعل from/to صحيحين وفي السورة نفسها، وعدد الآيات المطلوب يساوي completeAyahs بالضبط ما أمكن.
- إذا كان type=audio فحدد حد بداية وحد نهاية حقيقيين لصيغة «سجّل تلاوتك من قوله تعالى … إلى قوله تعالى …»، ويجب أن يساوي عدد الآيات من from إلى to قيمة reciteAyahs بالضبط.
- كل صورة تعرض مقتطفاً مستقلاً من 3 إلى 7 كلمات فقط من آية الحد، بلا أي آيات محيطة؛ لذلك لا تنسخ نص البداية أو النهاية داخل prompt ولا تكشف الجزء المطلوب إجابته.
- لا تنشئ سؤالاً سهلاً أو مباشراً، ولا تعتمد على مجرد معرفة اسم السورة إذا لم يتضمن السؤال تشابهاً لفظياً دقيقاً.
  ...
  try {
    ...
    if (mode === "generate_exam") {
      ...
      const allNumbers = Array.from({ length: endSurahNumber - startSurahNumber + 1 }, (_, index) => startSurahNumber + index)
        .filter((number) => SURAH_AYAH_COUNTS[number - 1] >= 20)
      if (!allNumbers.length) return json({ error: "لا توجد في النطاق المحدد سورة مؤهلة من 20 آية فأكثر", diagnostics }, 400)
      ...
      const safeQuestions = questions.slice(0, requestedCount).map((question: any) => {
        ...
        const defaultPrompts: Record<string, string> = {
          mcq: "ميّز الموضع القرآني الصحيح من خلال المقتطف المصور",
          truefalse: "حدّد صحة العبارة من خلال المقتطف القرآني المصور",
          complete: "اكتب من قوله تعالى إلى قوله تعالى",
          audio: "سجّل تلاوتك من قوله تعالى إلى قوله تعالى",
        }
        ...
        return {
          ...
          options,
          correct,
          startAnchor: { surahNumber: source.surahNumber, ayah: from, edge: "start", words: 7 },
          endAnchor: { surahNumber: source.surahNumber, ayah: to, edge: "end", words: 7 },
          questionImage: `/api/quran-question-image?surah=${source.surahNumber}&ayah=${from}&type=${type}&edge=start&words=7`,
          questionImageStart: `/api/quran-question-image?surah=${source.surahNumber}&ayah=${from}&type=${type}&edge=start&words=7`,
          questionImageEnd: `/api/quran-question-image?surah=${source.surahNumber}&ayah=${to}&type=${type}&edge=end&words=7`,
