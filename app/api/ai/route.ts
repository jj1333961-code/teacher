...
قواعد الدرجة:
- يُحسب score أساساً من نسبة الآيات المكتملة: (عدد الآيات الكلية - الآيات الناقصة) / عدد الآيات الكلية.
...
3) اكتب التعديل بنفس أسلوب بنية المشروع (نفس التسمية، نفس المسافات البادئة، نفس نمط الدوال، اتجاه RTL العربي، ومتغيرات الأنماط الموجودة مثل var(--primary)).
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // 1.ب) مساعد Gemini لإشارات مكافحة الغش المحلية فقط؛ لا يستقبل فيديو أو صورًا خامًا.
    if (mode === "anti_cheat_assistant") {
      const signals = Array.isArray(payload.signals) ? payload.signals.slice(0, 12) : []
      if (!signals.length) return json({ error: "لا توجد إشارات محلية للتحليل", diagnostics }, 400)
      const text = await runText(
        `حلّل ملخص إشارات المراقبة المحلية التالي: ${JSON.stringify(signals)}`,
        "أنت طبقة مساعدة لفحص الغش. لا تصدر حكمًا نهائيًا ولا تعتبر إشارة واحدة غشًا. أعد JSON فقط بالشكل {\\\"assessment\\\":\\\"normal|review|likely\\\",\\\"reason\\\":\\\"سبب مختصر\\\"}. اعتمد على استمرار وتوافق إشارتين أو أكثر، واعتبر انقطاع الكاميرا مشكلة تقنية لا غشًا.",
        0.05,
      )
      return json({ result: text.trim(), diagnostics })
    }

    // 1.ج) المساعد الذكي للطالب/ولي الأمر (مع سياق بيانات الطالب)
    if (mode === "assistant") {
      ...
      const text = await runText(
        `${reference ? `${reference}\n\n` : ""}السؤال:\n${prompt.slice(0, 6000)}`,
        "أنت مساعد المنصة الذكي، مساعد عربي طبيعي ودقيق. أجب عن أي سؤال مسموح داخل المنصة أو خارجها، وأعط الأو��وية لبيانات المنصة فقط عندما تكون ذات صلة. اجعل طول الجواب على قدر السؤال: جواب مباشر وقصير للسؤال البسيط، وتفصيل منظم فقط عند طلبه. تعامل مع التحيات والعبارات الاجتماعية بصورة طبيعية؛ مثال: إذا قال المستخدم السلام عليكم فرد: وعليكم السلام ورحمة الله وبركاته 🥰 هل لديك سؤال؟ أنا في خدمتك! استخدم الرموز التعبيرية باعتدال في الحديث الودي فقط، وتجنبها في الإجابات العلمية أو الحساسة. استخدم لغة عربية بسيطة واحترافية ولا تكرر السؤال. في القرآن والمتشابهات استخدم مقتطفات المرجعين للتحقق عند توفها، لكن لا تحصر معرفتك فيهما، ولا تختلق آية أو معلومة.",
        typeof body.temperature === "number" ? body.temperature : 0.35,
      )
      return json({ result: text.trim(), diagnostics })
    }
