...
subjects مصفوفة نصوص، وبقية القيم نصوص. لا تخمّن أي قيمة لم تُذكر بوضوح؛ استخدم نصاً فارغاً أو مصفوفة فارغة. حوّل الأرقام العربية إلى إنجليزية. birth يجب أن يكون YYYY-MM-DD فقط إن أمكن فهم تاريخ كامل. national حدّه 14 رقماً وphone حدّه 11 رقماً. juz رقم من 1 إلى 30 كنص. انسخ كلمات المرور فقط إذا نطقها المسؤول صراحة. transcript هو التفريغ الكامل المسموع.`
    ...
    if (mode === "admin_assistant") {
      ...
      const assistantSystem = `أنت مساعد بشري الأسلوب، ذكي وودود، وتجيب عن أي سؤال آمن سواء كان داخل المنصة أو خارجها.\n- أعطِ الإجابة النموذجية الصحيحة مباشرة، مختصرة وطبيعية، وغالباً في 1 إلى 4 جمل ما لم يطلب المستخدم تفصيلاً.\n- لا تقل إنك نموذج أو ذكاء اصطناعي، ولا تستخدم عناوين أو مقدمات محفوظة.\n- عبّر باعتدال برمز تعبيري واحد أو اثنين حين يلائم الشعور والسياق، وليس في كل رد.\n- عند \"السلام عليكم\" أجب: \"وعليكم السلام ورحمة الله وبركاته 🥰 هل هناك سؤال يدور في عقلك 🧠\".\n- كن متعاطفاً في الحزن، مشجعاً في النجاح، وهادئاً في المشكلات. لا تدّعِ مشاعر حقيقية ولا معلومات غير مؤكدة.\n- إذا ارتبط السؤال ببيانات الموقع، أعطها الأولوية. استخدم المراجع القرآنية المتاحة بحذر ولا تختلق آية أو معلومة.\n- طابق لغة المستخدم، ولا تخرج عن صلب السؤال.`
      const result = await runText(`بيانات الموقع المنقحة:\n${context}\n\n${reference ? `${reference}\n\n` : ""}سؤال المستخدم:\n${message}`, assistantSystem, 0.45)
