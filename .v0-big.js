...
const LANG_DICT = {
  ...
  'أدخل اسم المستخدم والرقم السري — سيتم نقلك تلقائياً لصفحتك (مسؤول / طالب / ولي أمر).': 'Enter your username and password — you will be routed automatically to your page (admin / student / parent).',
...
function translateValue(value) {
  ...
  if(LANG_DICT[clean]) return value.replace(clean, LANG_DICT[clean]);
  let result = value;
  Object.keys(LANG_DICT).sort((a,b)=>b.length-a.length).forEach(function(key){ if(result.indexOf(key)!==-1) result=result.split(key).join(LANG_DICT[key]); });
  return result;
}
...
function toggleLang() { currentLang=currentLang==='ar'?'en':'ar'; localStorage.setItem('lang',currentLang); applyLangToDom(); }
...
function getData(key, def) { const d = localStorage.getItem(key); return d ? JSON.parse(d) : (def || []); }
...
async function hydrateDataFromNeon(){
  ...
  try{ const res=await fetch('/api/data',{cache:'no-store'}); const body=await res.json(); if(res.ok&&body.data){ Object.keys(body.data).forEach(function(key){ localStorage.setItem(key,JSON.stringify(body.data[key])); }); sessionStorage.setItem('neon_hydrated_v1','1'); location.reload(); } else { sessionStorage.setItem('neon_hydrated_v1','1'); await saveAllDataToNeon(); } }
...
document.addEventListener('DOMContentLoaded',()=>setupProctorHold(document.getElementById('proctorGateHold'),true));
...
function updateSurahSelect() {
  ...
  if(hint) hint.textContent='حدود الج��ء: '+ranges.map(r=>'سورة '+(ALL_SURAHS_ORDERED[r[0]-1]||r[0])+' '+r[1]+'–'+r[2]).join(' | ');
}
...
async function renderGoogleButton() {
  ...
  if(!container) return;
  container.innerHTML = '';
  box.innerHTML = '<div class="alert alert-info">جارٍ تحميل تسجيل الدخول بحساب Google...</div>';
  await loadGoogleClientId();
  if(!initGoogleGsi()) {
    box.innerHTML = '<div class="alert alert-danger">تعذر تفعيل تسجيل الدخول بحساب Google من الخادم. يرجى مراجعة إعداد GOOGLE_CLIENT_ID في Vercel.</div>';
    return;
  }
  box.innerHTML = '';
  google.accounts.id.renderButton(container, { theme: 'filled_blue', size: 'large', type: 'standard', shape: 'pill', text: 'signup_with', locale: 'ar' });
  // إظهار نافذة اختيار حسابات جوجل الموجودة على الجهاز تلقائياً
...
function handleGoogleCredential(resp) {
  ...
  if(!payload || !payload.email) { box.innerHTML = '<div class="alert alert-danger">❌ تعذّر قراءة بيانات حساب جوجل، حاول مرة أخرى</div>'; return; }
...
function sendSignupCode() {
  ...
  const text = 'طلب كود تحقق لحساب جديد في نظام إدارة الطلاب\n'
    ...
    + 'يرجى إرسال هذا الكود لمقدّم الطلب لإ��مال التسجيل.';
...
let studentIntakeRecorder=null, studentIntakeChunks=[];
...
async function toggleStudentIntakeRecord(){
  ...
  try{
    ...
    recorder.onstop=async function(){
      const blob=new Blob(studentIntakeChunks,{type:recorder.mimeType||mimeType||'audio/webm'});studentIntakeChunks=[];
      try{
      ...
      }catch(e){result.innerHTML='<div class="alert alert-danger">تعذر تحليل التسجيل: '+escapeHtml(e.message||'خطأ غير معروف')+'<br><button type="button" class="btn btn-sm btn-primary" onclick="toggleStudentIntakeRecord()">إعادة التسجيل</button></div>';status.textContent='فشل التحليل';}
...
function editAdmin(id) {
  ...
  if(newMobile.length !== 11) return alert('رقم الم��بايل يجب أن يكون 11 رقم');
...
// ====== شاشة القفل الموحدة: توجيه لقائي للصفحة المناسبة ======
...
async function saveStudent() {
  ...
  if(students.find(s => s.username === username)) return fail('اسم المستخدم مسجل مسبقاً');
  // ✅ مسموح الآن أن يكون ال��قم السري للطالب مطابقاً للرقم السري لولي الأمر
  ...
  document.getElementById('voiceRecordStatus').textContent = 'اضغط للتسجي�� (20 ثانية)';
...
function openEdit(id) {
  ...
  const evs=document.getElementById('editVoiceStatus'); if(evs)evs.textContent=(s.voiceProfile||s.voicePrintVec)?('البصمة الصوتية موجودئ'+(s.voiceProfile?' (جميناي)':'')+'. يمكنك استبدالها بتسجيل جديد.'):'لا توجد بصمة صوتية محفوظة حالياً.';
  ...
  try{
    ...
    editVoiceRecorder.onstop=async()=>{
      ...
      try{
        ...
        status.textContent='تم إنشاء بصمة جديد�� بواسطة Gemini — اضغط حفظ التعديلات';
...
function renderRecordElementHTML(el, i, num) {
  ...
  html += '<button class="btn btn-xs btn-info" onclick="duplicateRecordElement('+i+')" title="تضعيف (إنشاء نسخة أخرى)">📑 تضعيف</button>';
...
async function readApiJson(res, fallback){
  ...
  if(!text.trim())throw new Error(fallback+' — لم يرسل الخادم تفاصيل (HTTP '+res.status+'). أعد المحاولة، وإن استمر الخطأ تحقق من سجل النشر.');
  let data=null;
  ...
  if(!data)throw new Error(fallback+' — است��ابة غير صالحة (HTTP '+res.status+').');
  if(!res.ok){const reason=data.error||fallback;const stage=data.diagnostics&&data.diagnostics.stage?' — المرحلة: '+data.diagnostics.stage:'';const error=new Error(reason+stage+' (HTTP '+res.status+')');error.status=res.status;error.code=data.code||'';error.retryable=data.retryable===true;throw error}
  return data;
}
...
function localSmartChatReply(message,role){
  ...
  const roleLabel=role==='admin'?'المسؤول':role==='parent'?'ولي الأمر':'الطئلب';
  ...
  if(/طالب|طلاب|اختبار|نتيج|درج|تسميع|حفظ|مراجع/.test(q)){
    ...
    return 'لتحسين الحفظ: ابدأ بمراجعة قصيرة للمقطع القريب، ثم اختبر نفسك عشوائياً من مق��ع أقدم، وسجّل المواضع التي توقفت فيها. كرر المواضع الضعيفة ثلاث مرات ثم أعد الاختبار دون النظر إلى المصحف.';
  }
  if(/وقت|تنظيم|خطه|خطة|جدول|فكره|فك��ة/.test(q))return 'خطة مقترحة: 10 دقائق للماضي القريب، 10 دقائق للماضي البعيد، 5 دقائق لأسئلة عشوائية من أول ووسط وآخر السور، ثم دقيقتان لتسجيل الأخطاء. اجعل الهدف محدداً بعدد آيات أئ سور، لا بمدة فقط.';
  ...
  if(/صعب|ضعف|نسي|نس��ان|خطا|خطأ/.test(q))return 'عند وجود ضعف، لا تُعد السورة كاملة مباشرة. حدّد موضع الخطأ، اقرأ ما قبله وما بعده، اربطه بأول كلمة في الآية التالية، ثم اختبر الموضع من بداية مختلفة. أعد مراجعته اليوم وبعد يوم وعد أسبوع.';
  return 'بصفتي المساعد المحلي لـ'+roleLabel+'، أستطيع تقديم جواب أدق إذا ذكرت الهدف والسورة أو النتيجة أو المشكلة الحالية. سأحوّلها إلى خطوات واضحة قابلة للتنفيذ دون ادعاء معلومات غير موجودة في المنصة.';
}
async function sendAdminChat(){const input=document.getElementById('adminChatInput'),box=document.getElementById('adminChatMessages'),message=input.value.trim();if(!message||currentType!=='admin')return;input.value='';box.innerHTML+='<div class="ai-msg user">'+escapeHtml(message)+'</div><div id="adminAiTyping" class="ai-msg bot">جاري التحليل...</div>';box.scrollTop=box.scrollHeight;try{const students=getData('students',[]).map(s=>({id:s.id,name:s.name,parent:s.parent,juz:s.juz,surah:s.surah,examResults:(s.examResults||[]).slice(-5),activeExam:s.activeExam?{status:s.activeExam.status,date:s.activeExam.date}:null}));const res=await fetch('/api/ai',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mode:'admin_assistant',payload:{role:'admin',message,context:{students,studentCount:students.length,messageCount:getData('messages',[]).length}}})});const data=await readApiJson(res,'تعذر رد OpenRouter');document.getElementById('adminAiTyping').outerHTML='<div class="ai-msg bot">'+escapeHtml(data.result).replace(/\n/g,'<br>')+'</div>'}catch(e){document.getElementById('adminAiTyping').textContent=localSmartChatReply(message,'admin')}box.scrollTop=box.scrollHeight}
...
function syncExamSurahRange(){
  ...
  try{const range=getExamPastRange(),help=document.getElementById('examPastScopeHelp');if(help)help.textContent='القريب: '+range.near[0]+' ← '+range.near[range.near.length-1]+(range.far.length?'، البعيد: '+range.far[0]+' ← '+range.far[range.far.length-1]:'، و��ا يوجد نصف بعيد مستقل في هذا النطاق.')}catch(e){}
}
...
function renderExamQuestions(){
  ...
  h+='<div class="exam-question exam-slide-panel '+(q.rejected?'rejected':'')+'">'+
    ...
    '<div class="form-group"><label>السورة</label><input value="'+escapeHtml(q.surah||'')+'" onchange="updateExamQuestion('+i+',\'surah\',this.value)"><small style="color:var(--text-light)">حدود الآي��ت محفوظة داخلياً للصورة والتصحيح ولا تظهر كخانات في السؤال.</small></div>';
  ...
  else h+='<div class="alert alert-info">سيتم التحقق من بصمة الطالب أولاً، ثم من محتوى التلاوة. إذا كانت البصمة ��ير مطابقة فلن يُحفظ ا��تسجيل.</div>';
  h+='</div>';c.innerHTML=h;
}
...
function saveExamDraft(){
  ...
  if(i<0||!qs.length){showExamAlert('أضف سؤالاً ��احداً على الأقل قبل الحفظ.','danger');return}
...
function renderStudentExam(){
  ...
  if(!ex||ex.status!=='pending'){const latest=(s&&s.examResults||[]).slice(-1)[0];c.innerHTML=latest?renderStudentResultHtml(latest):'<div class="alert alert-info">لا يوجد اختبار نشط حالياً.</div>';return}
...
async function recordStudentExamAudio(i){
  ...
  try{
    ...
    recorder.onstop=async()=>{
      ...
      aiBox.innerHTML='<div class="alert alert-info">🔒 تم حفظ إجابة التسجيل ونتيجة التحليل داخلياً. لن تظهر نتيجة الذكاء الاصطناعي للطالب حتى يقرر المسؤول نشرها.</div>';
...
function renderStudentResultHtml(ex){if(ex.status==='pending_audio_review')return '<div class="alert alert-warning"><h3>تم استلام الاختئار</h3><p>النتيجة معلّقة لحين مراجعة المئؤول للتسجيل الصوتي.</p></div>';let h='<div class="alert alert-success"><h3>النتيجة النهائية: '+ex.score+' / '+ex.maxScore+'</h3><p>تم حفظ جميع إجاباتك وإرسالها للمسؤول.</p></div>';if(ex.questions&&ex.answers){h+='<details open><summary>مراجعة إجاباتك</summary>';ex.questions.forEach((q,i)=>{const a=ex.answers[i]||{};h+='<div class="task-card"><strong>س'+(i+1)+':</strong> '+escapeHtml(q.prompt||'')+'<br>إجابتك: '+escapeHtml(a.answer||'—')+'<br>التقييم: '+(a.score>=1?'صحيحة':'غير صحيحة')+'</div>'});h+='</details>'}return h}
...
function renderParentExamResults(s){
  ...
  arr.slice().reverse().forEach(ex=>{h+='<div class="history-element"><div class="history-element-name">📅 '+escapeHtml(ex.date||'')+'</div><div class="history-element-details"><div class="history-detail"><strong>الدرجة:</strong> '+ex.score+'/'+ex.maxScore+'</div><div class="history-detail"><strong>الوقت:</strong> '+ex.totalDurationSeconds+' ثانية</div></div>';
    if(ex.answers&&ex.questions){h+='<details style="margin-top:10px"><summary>👁️ عرض إجابات الطالب</summary>';ex.questions.forEach((q,i)=>{const a=ex.answers[i]||{},r=a.aiResult||{};h+='<div class="task-card"><strong>س'+(i+1)+':</strong> '+escapeHtml(q.prompt||'')+'<br><span>إجابة الطالب: '+escapeHtml(a.answer||'—')+'</span><br><span>النتيجة: '+(a.score>=1?'✅ كاملة':a.score===.5?'🟡 نصف درجة':'❌ غير صحيحة')+'</span>'+(r.reason?'<br><span>تقرير AI: '+escapeHtml(r.reason)+'</span>':'');if(q.type==='audio'&&a.audioData&&a.audioShareWithParent!==false){h+='<div style="margin-top:8px">🎙️ التسجيل: <audio controls src="'+a.audioData+'" style="height:38px"></audio></div>'}h+='</div>'});h+='</details>'}h+='</div>'});
  return h+'</div>';
}
  ...
  if(finalizedSessions.length === 0) {
    ...
    Object.keys(grouped).sort().reverse().forEach(date => {
      ...
      grouped[date].forEach(sess => {
        ...
        if(sess.finalizedAt) html += '<p style="margin-top:8px; color:var(--success); font-size:0.9rem;">✅ تم الإغلاق النهائي تاريخ: '+sess.finalizedAt+'</p>';
...
async function refreshGithubSync(){
  ...
  if(box) box.innerHTML = '<p style="color:var(--text-light)">جارٍ التحقق من التال بمستودعك...</p>';
  try {
    ...
    if(btn){ btn.disabled = false; btn.textContent = '🔄 تحقق من حالة المزامنة'; }
  }
}

async function deleteGithubFile(){
  if(currentType !== 'admin'){ showToast('❌ ذه الميزة متئحة للمسؤول فقط', 'error'); return; }
...
// ===== مساعد تطوير الموقع بالذكاء الاصطناعي (للمسؤول فقط) =====
...
// عبارات تدل على عمليات خطيرة تتطلب تأكيداً صريحاً قبل التنفي اللقائي.
const DEV_DANGER_PATTERNS = [
  {re:/ئذف\s*(قاعدة|لقاعدة)\s*البيانات|drop\s+database|delete\s+database/i, label:'حذف قاعدة البيانات'},
...
function clearDevAudit(){
  if(!confirm('هل تريد مسح سجل الت��قيق بالكامل؟ لا يمكن التراجع عن هذا الإجراء.\nملاحظة: تاريخ Git على GitHub يبقى محفوظاً ولا يتأثر.')) return;
  setData('devAuditLog', []);
  renderDevAudit();
  showToast('تم مسح سجل التدقيق', 'info');
}
...
async function runDevAssistant(){
  ...
  if(dangers.length){
    ...
    if(!confirmed){
      recordDevAudit({ status:'blocked', request, summary:'أُلغي بواسطة المسؤول قبل التنفيذ.', flagged:dangers });
      renderDevAudit();
  ...
  resultBox.innerHTML = '<p style="color:var(--text-light)">جارٍ ت��ليل المشروع وقراءة الملفا�� البرمجية اللازمة ثم تطبيق التعديل...</p>';
  ...
  try {
    ...
    if(progressLabel)progressLabel.textContent='اكتمل التنفي��';
...
function renderDevPlan(plan){
  ...
  if(plan.applied){
    ...
    // 2) ملخص التغييرات
    h += '<div style="margin-bottom:6px;"><strong>🧾 ملخص التغييرات:</strong> '+esc(plan.summary || 'تم تطبيق التعديلات البرمجية المطلوبة بنجاح.')+'</div>';
    ...
    if(Array.isArray(plan.applied) && plan.applied.length){
      ...
      plan.applied.forEach(function(a){
        h += '<li><code style="direction:ltr;background:rgba(0,0,0,0.06);padding:2px 6px;border-radius:5px;">'+esc(a.path||'')+'</code>'+
             ...
             (a.commitUrl ? ' — <a href="'+esc(a.commitUrl)+'" target="_blank" rel="noopener">عرض ال��commit</a>' : '')+'</li>';
      });
      h += '</ul>';
    }
...
function toggleShareWithParent(idx) {
  ...
  setData('messages', msgs);
  renderMessages();
  alert(msgs[idx].shareWithParent ? '✅ أصبح بإمكان ولي الأمر الاطلاع على هذا الملف (بدون تعديل).' : '🚫 تم منع ولي الأر من الاطلاع على هذا الملف.');
}
...
function replyMessage(idx) {
  ...
  showToast('✅ تم إرسال الر', 'success');
  setData('messages', msgs);
  renderMessages();
}
...
function editSubject(id) {
  ...
  if(newPhone && newPhone.length !== 11) return alert('رقم الهاتف يجب أن يكون 11 رقم');
...
function renderStudentDashboard() {
  ...
  if(draft && isQuran) {
    let draftHtml = '<div class="page draft-card" style="margin-top:20px;"><h4 style="color:var(--warning); margin-bottom:15px;"> تسميع اليوم (مسودة - قيد التعديل)</h4>';
    ...
    draft.elements.forEach((el, ei) => {
      ...
      draftHtml += 'التقييم: <span class="badge '+getRatingClass(el.rating)+'">'+getRatingLabel(el.rating)+'</span>';
      draftHtml += '</div>';
    });
    const timeLeft = Math.max(0, 24 - ((Date.now() - draft.draftCreatedAt) / (60 * 60 * 1000)));
...
function renderStudentCompletedTasks() {
  ...
  completed.slice().reverse().forEach(task => {
    ...
    html += '<p style="color:var(--text-light); font-size:0.9rem;">🕐 تمت الموافقة: '+task.approvedAt+'</p>';
    html += '</div>';
  });
  html += '</div>';
  html += renderTaskArchiveHtml(s);
...
function renderStudentTasks() {
  ...
  allTasks.forEach((task, originalIdx) => {
    ...
    if(task.type === 'homework') {
      html += '<div class="task-card" data-task-category="extra" '+cardStyle+'><h5>📝 واجب: '+(task.name || task.text || '')+'</h5>';
      if(task.surah) html += '<p>السورة: '+(task.surah || '-')+' | من آية '+(task.from || '-')+' إلى آية '+(task.to || '-')+'</p>';
      html += studentAyatBlock(task, 'hw'+originalIdx);
      html += status;

      if(isRejected) {
        html += '<div class="task-rejected-alert">⚠️ تم رفض الملف السابق. يرجى إرسال ملف جديد.</div>';
        html += '<div style="margin-top:10px;"><div class="file-upload" onclick="document.getElementById(&quot;hwFile_'+originalIdx+'&quot;).click()"><div>📷 اضغط لرفع صورة الواجب الجديدة</div></div>';
        ...
        html += '<div style="margin-top:10px;"><div class="file-upload" onclick="document.getElementById(&quot;hwFile_'+originalIdx+'&quot;).click()"><div>📷 اضغط لرفع صورة الواجب</div></div>';
        html += '<input type="file" id="hwFile_'+originalIdx+'" accept="image/*" style="display:none" onchange="uploadTaskFile('+originalIdx+', this, &quot;homework&quot;)"></div>';
      } else {
      ...
      if(isProctorCancelled) {
        html += '<div class="task-rejected-alert">لا يمكن إعادة هذه المهمة بعد إلغائها. تواصل مع المسؤول لإسناد مهمة جديدة.</div>';
      } else if(isRejected) {
  ...
  document.getElementById('studentTasksSection').innerHTML='<div class="student-work-grid" style="margin-top:20px"><section class="page student-work-card"><h4 style="color:var(--primary)">الاختبارات <span class="badge badge-primary">'+(exam?1:0)+'</span></h4>'+examHtml+'</section><section class="page student-work-card"><h4 style="color:var(--success)">التسميع <span class="badge badge-success">'+(recitationTasks?temp.querySelectorAll('[data-task-category="recitation"]').length:0)+'</span></h4>'+(recitationTasks||'<p style="color:var(--text-light)">لا توجد مهمات تسميع حالية.</p>')+'</section><section class="page student-work-card"><h4 style="color:var(--warning)">المهمات الإضافية <span class="badge badge-warning">'+(extraTasks?temp.querySelectorAll('[data-task-category="extra"]').length:0)+'</span></h4>'+(extraTasks||'<p style="color:var(--text-light)">لا توجد مهمات إضافية حالية.</p>')+'</section></div>';
...
async function serverRecitationAnalysis(blob, task) {
  try {
    ...
    return {analysisError:e&&e.message?e.message:'تعذر تحليل التسجيل على الخادم'};
  }
}
...
// التحقق الكامل (بصمة صوتية + محتوى ائتلاوة) ثم الإرسال أو الرفض
async function verifyAndSubmitRecitation(taskIdx, blob, dataUrl, transcript, aiBoxId, statusEl, fileName) {
  ...
  if(matchPct === null) {
    showToast('❌ لا توجد بصمة صوتية مرجعية صالحة لهذا الطالب — لم يتم حفظ أو إرسال التسجيل', 'error');
  ...
  if(matchPct !== null && matchPct < VOICE_MATCH_THRESHOLD) {
    showToast('❌ فشل التحقق من البصمة الصوتية (' + matchPct + '%) — لم يتم حفظ أو إرسال التسجيل', 'error');
    if(statusEl) statusEl.textContent = 'غير مطابق — ��عد التسجيل ❌';
  ...
  if(aiBox) aiBox.innerHTML = '<div class="alert alert-success"><strong>✅ تقرير الذكاء الاصطنعي:</strong><br>' +
...
// رفع ملف تسجيل صوتي جاهز من جهاز الطالب (يمر بنفس التحقق)
...
async function computeVoicePrint(blob) {
  // بصة محلية متعددة السم��ت: طيف صوتي + مركز الطيف + عبور الصفر + إحصاءات زمنية.
  // لا نرفع البصمة الخام إلى OpenRouter؛ تبقى المقارنة على جهاز المستخدم.
  try{
...
async function verifyVoiceIdentity(blob,student){
  ...
  if(!profile)throw new Error('لا توجد بصمة Gemini مرجعية محفوظة لهذا الطالب');
  const audio=await voiceAudioPayload(blob);
...
// OpenRouter يقبل صوت الإدخال بصيغة wav أو mp3 فقط، بينما المتصفح يسجّل غالباً بصية webm.
...
async function sendStudentChat() {
  ...
  const chatDiv=document.getElementById('studentChatMessages'); chatDiv.innerHTML+='<div class="ai-msg user">'+escapeHtml(text)+'</div><div id="studentAiTyping" class="ai-msg bot">⏳ جاري الئفكير...</div>';chatDiv.scrollTop=chatDiv.scrollHeight;
  ...
  try{
    ...
    const data=await readApiJson(res,'تعذر رد OpenRouter');
    const reply=escapeHtml(data.result||'لم يصل رد.').replace(/\n/g,'<br>'); const typing=document.getElementById('studentAiTyping');if(typing){typing.id='';typing.innerHTML=reply}else chatDiv.innerHTML+='<div class="ai-msg bot">'+reply+'</div>';
  }catch(e){const typing=document.getElementById('studentAiTyping');if(typing)typing.innerHTML=escapeHtml(localSmartChatReply(text,'student'))+'<br><small>رد محلي محدود بسبب تعذر OpenRouter.</small>'}
...
async function sendParentChat() {
  ...
  const chatDiv=document.getElementById('parentChatMessages');chatDiv.innerHTML+='<div class="ai-msg user">'+escapeHtml(text)+'</div><div id="parentAiTyping" class="ai-msg bot"> جاري تحليل البيانات...</div>';chatDiv.scrollTop=chatDiv.scrollHeight;
  ...
  try{
    ...
    const data=await readApiJson(res,'تعذر رد OpenRouter');const reply=escapeHtml(data.result||'لم يصل رد.').replace(/\n/g,'<br>');const typing=document.getElementById('parentAiTyping');if(typing){typing.id='';typing.innerHTML=reply}else chatDiv.innerHTML+='<div class="ai-msg bot">'+reply+'</div>';
  }catch(e){const typing=document.getElementById('parentAiTyping');if(typing)typing.innerHTML=escapeHtml(localSmartChatReply(text,'parent'))+'<br><small>رد محلي محدود بسبب تعذر OpenRouter.</small>'}
  chatDiv.scrollTop=chatDiv.scrollHeight;
}
  ...
  children.forEach(s => {
    ...
    if(pendingTasks.length === 0) {
      ...
      pendingTasks.forEach((task, i) => {
        const sentTime = task.sentAt ? new Date(task.sentAt).toLocaleString('ar-EG') : 'غير محدد';
...
function renderParentFullChart() {
  ...
  if(finalizedSessions.length === 0) {
    document.getElementById('parentChartContainer').innerHTML = '<div class="alert alert-info">لا توجد بيانات كافية للمخطط</div>'; return;
  }
...
function uploadFile(input) {
  ...
  reader.onload = function(e) {
    ...
    const fileData = {
      ...
      uploadedBy: currentUser ? (currentUser.mobile || currentUser.name || 'مسؤول') : 'مسؤول'
    };
    files.push(fileData);
...
function renderFiles() {
  let files = getData('uploadedFiles', []);
  const categoryLabels = {student:'👨‍🎓 الطالب', parent:'👨‍👩‍👧 ولي الأمر', general:'عام', quran:'قرآن', lessons:'دروس', exams:'اختبارات', reports:'تقارير'};
...
function fileTargetLabel(f) {
  ...
  if(!f.targetId || f.targetId === 'all') return aud === 'parent' ? 'كل أولياء الأمور' : 'كل الطلاب';
  const st = getData('students', []).find(x => String(x.id) === String(f.targetId));
  if(!st) return 'مستلم محدد';
...
function generateAIReport(student) {
  ...
  let report = '';
  if(avgScore >= 14) report = 'ممتاز يا '+student.name+'! 🌟 مستواك رائع جداً. أنت تحفظ بثبات وتميز. استمر في المراجعة وستكون من حفاظ كتاب الله.';
...
function generateWelcomeMessages(student) {
  ...
  const templates = [
    {title: 'هلاً بك يا '+student.name+'! 🌟', body: 'يوم جديد، فرصة جديدة للتقرب من كتاب الله. اجعل لنفسك ورداً يومياً لا يفوتك، فالقرآن نور يُهدى به الله من يشء.'},
...
function generateParentWelcome(student) {
  ...
  if(last) {
    ...
    if(last.totalScore >= 14) base.body += 'أداء ممتاز! ابنك يُظهر تفوقاً واضحاً. استمر في تشجيعه. 🌟';
...
function generateAIResponse(text, student) {
  ...
  if(has('تقد','تطور','مقارنة','احصائ','إحصائ','رسم','مخطط')) {
    if(finalizedSessions.length < 2) return ' أحتاج تسميعين نهائيين على الأقل لأقارن تقدمك. سجّل تسميعك القادم وسأحلل لك المنحنى بدقة.';
    ...
    r += '• آخ تغيّر: '+(diff>0?('ارتفاع بمقدار '+diff+' درجة 🎉'):(diff<0?('انخفاض بمقدار '+Math.abs(diff)+' درجة — لا بأس، عوّضها بمراجعة إضافية 💪'):'ثبات ��ي المستوى '));
    return r;
  }
  // المهام / الواجب
  if(has('واجب','مهام','مهمة','مطلوب','تسجيل صوتي','راءة')) {
    if(pending.length === 0) return '🎉 لا توجد مهام معلّقة الآن يا '+name+'. استغل الوقت في مراجعة ما حفظته سابقاً.';
    ...
    r += '<br>💡 المهام التي عليها علائة 👁️ يمكنك عرض آياتها كصورة واضحة مع إمكانية التكبير.';
    return r;
  }
  // الآيات والصور
  if(has('اية','آية','ايات','آيات','صورة','اقرأ','مصحف')) {
    return '📖 لعرض الآيات المطلوبة منك:<br>1. افتح <strong>المهام المطلوبة</strong> في صفحتك.<br>2. اضغط <strong>📖 عرض الآيات بحجم كبير</strong> في المهمة.<br>3. استخدم زرار ➕ / ➖ للتكبير والتصغير حتى تصل لأوضح حجم لعينيك.<br><br>الآيات تُعرض بارم العثماني المشكَّل كصورة مطابقة تماماً لمصحف.';
  }
  // البصمة الصوتية
  if(has('بصمة','صوتي','صوت','ميكروفون','تحق')) {
    return '🎙️ <strong>البصمة الصوتية:</strong> عند تسجيلك أول مرة حُفظت بصمة صوتك في النظام. عند إرسالك أي تسجيل، يحلله الذكاء الاصطناعي ويطابقه ببصمتك تلقائياً، ولا يُقبل التسجيل إلا إذا كان صوتك أنت.<br><br>لأفضل نتيجة: سجّل في مكان هادئ، وقرّب الميكروفون، وتحدث بصوت طبيعي واضح.';
  }
  // خطة الحفظ
  if(has('خطة','جدول','تنظيم','وقت','كيف احفظ','كيف أحفظ')) {
    return '🗓️ <strong>خطة يومية مقترحة لك '+name+':</strong><br>• بعد الفجر (20 د): حفظ جديد — كرر الآية 7 مرات نظراً ثم 3 مرات غيباً.<br>• بعد العصر (10 د): تثبيت حفظ اليوم.<br>• قبل النوم (15 د): مراجعة حفظ الأمس + صفحة قديمة.<br>• يوم الجمعة: مر��جعة شاملة لكل ما حفظته في الأسبوع.<br><br>الاستمرار القليل الدائم خير من الكثير المنقطع.';
  }
  // نصائح
  if(has('نص��حة','نصائح','ساعدني','مساعدة','انسى','أنسى','نسيت','صعب')) {
    return '💡 <strong>خمس قواعد ذهبية للحفظ:</strong><br>1. اربط الحفظ بوقت ثابت لا يتغير.<br>2. اقرأ الآية بصوت مسموع — السمع يثبّت أضعاف النظر.<br>3. افهم معنى الآية قبل حفظها.<br>4. لا تنتقل لآية جديدة قبل إتقان ما قبلها.<br>5. راجع، ثم راجع، ثم راجع — النسيان طبيعي والمراجعة علاجه.';
  }
  // تحفيز
  if(has('تحفيز','همة','ملل','تعبان','زهقان','احبت','أحبطت')) {
    const quotes = [
      '🌟 «وَلَقَدْ يَسَّرْنَا الْقُْآنَ لِلذِّكْرِ فَهَلْ مِن مُّدَّكِرٍ» — الحفظ أيسر مما تظن، ابدأ فقط.',
  ...
  // التواصل مع المسؤو
  if(has('مسؤول','معلم','شيخ','ابلاغ','إبلاغ','رساله','رسالة','تواصل','شكوى')) {
    return '📨 يمكنك مراسلة المسؤول مباشرة من <strong>صندوق الرسائل</strong> في صفحتك، وسيصلك الرد هناك مع إشعار. إن كان الأمر عاجلاً اذكر كلمة "عاجل" في بداية رسالتك.';
  }
  // ديني عام
  if(has('دئن','اسلام','إسلام','الله','نبي','رسول','دعاء','صلاة')) {
    return 'الحمد لله رب العالمين 🌿<br><br>القرآن كتائ الله المعجز، زل هداية للناس. ومن الأدعية النافعة للحفظ: «اللهم اجعل القرآن ربيع قلبي ونور صدري وجلاء حزني وذهاب همّي».';
  }
  // افتراضي ذكي
  let r = '🤖 أهلاً '+name+'، لم فهم سؤالك تماماً، لكني أستطيع مساعدتك فوراً في:<br>';
  r += '• <strong>مستواي</strong> — تحليل آخر تقييم<br>• <strong>مهامي</strong> — الواجبات والتسجيلات<br>• <strong>تقدمي</strong> — إحصائيات وتطورك<br>• <strong>خطة</strong> — جدول حفظ يومي<br>• <strong>الآيات</strong> — كيف تعرض آيات التسميع كصورة<br>• <strong>تحفيز</strong> — كلمة تشدّ همتك';
